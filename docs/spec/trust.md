...
Jupyter connection keys, authorization tokens, environment variables, cell contents, stdin responses, transferred file bytes, and rendered HTML are sensitive. Default logs contain identifiers and bounded summaries, not raw secret-bearing payloads. The remote connection UI accepts tokens through hidden input and retains its profile only in Neovim process memory; disconnect clears the override. Remote REST redirects and file traversal components are rejected; resolved tokens travel only over the local Neovim-to-sidecar RPC and authenticated requests to the configured origin, and are never echoed in responses.

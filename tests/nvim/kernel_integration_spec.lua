...
test("shares one namespace across cells containing an indented %time magic", function()
	...
	set_source(magic_cell, "for _ in range(1):\n    %time magic_value = shared_env + 1\nprint(magic_value)")
	local before = kernel.status(state)
	kernel.run_cells(state, { cell, magic_cell })
	wait_for(function()
		return cell.execution_status == "completed" and magic_cell.execution_status == "completed"
	end, "cross-cell magic execution did not complete", 40000)
	local after = kernel.status(state)
	assert(after.notebook_id == before.notebook_id)
	assert(after.generation == before.generation)
	assert(output_text(magic_cell):find("CPU times:", 1, true))
	assert(output_text(magic_cell):find("42", 1, true))
	state:delete_cell(2)
end)
